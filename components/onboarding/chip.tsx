...
export function ChipWrap({ children }: { children: ReactNode }) {
  return <ul className="flex flex-wrap justify-center gap-2">{children}</ul>;
}
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="flex size-8 shrink-0 cursor-pointer items-center justify-center rounded-full text-accent-500 transition-colors hover:bg-accent-500/20 pointer-coarse:size-9"
      >
  ...
  return (
    <li>
      <button
        ...
        onClick={onPress}
        aria-label={label}
        className={cx(CHIP_BASE, "py-2 pl-3", CHIP_IDLE)}
      >
        <RiAddLine className="size-4 shrink-0 text-text-tertiary" aria-hidden />
        <span className="flex flex-col items-start text-left">
          <span>{children}</span>
          {sublabel ? (
            <span className="max-w-[22rem] truncate text-caption-2-regular text-text-secondary">
              {sublabel}
            </span>
          ) : null}
        </span>
